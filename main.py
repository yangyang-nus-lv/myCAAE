...
assert tuple(int(ver_num) for ver_num in torch.__version__.split('.')) >= (0, 4, 1),\
...
if __name__ == '__main__':
    ...
    hp.NUM_Z_CHANNELS = args.z_channels
    net = model.CAAE()
    ...
    if args.mode == 'train':
        ...
        betas = (args.b1, args.b2) if not args.load else None
        weight_decay = args.weight_decay if not args.load else None
        ...
        image_tensor = pil_to_tensor_transforms(pil_loader(args.input)).to(net.device)
        net.test_single(
